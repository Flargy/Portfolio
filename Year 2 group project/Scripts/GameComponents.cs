...
public class GameComponents : MonoBehaviour
{
    ...
    private void Start()
    {
        //StartCoroutine(UppdatePlayerPosition());
    ...
    /// </summary>
    /// <returns></returns>
    private IEnumerator SpawnAGnome() //ev flytta till cow follow state
    {
        float rand = 0f;
        while (true)
        {
            rand = Random.Range(0f, 100f);
            if (FollowingCowsList.Count > 0 && day && rand > 70f) //tog bort random effekten
            {
                foreach (GameObject cow in FollowingCowsList)
                {
                    ...
                    Physics.SphereCast(newSpawnPoint + new Vector3(0, 100, 0), 1f, Vector3.down, out groundCheck, Mathf.Infinity, LayerMask.GetMask("Default"));
                    
                        Debug.Log("raycast hit something at: " + groundCheck.point);
                        Debug.Log(groundCheck.point.y);
                        newSpawnPoint.y += groundCheck.point.y;
                        GameObject newGnome = Instantiate(gnomePrefab, newSpawnPoint, Quaternion.identity);
                        newGnome.GetComponent<GnomeStateMachine>().SetGnomeTarget(cow);
